...
//
// Density pass (explicitly requested) on all 3 variants below: image
// heights/avatar sizes trimmed one step, internal padding/gaps tightened
// one step on the existing spacing scale (no new values introduced),
// nothing shrunk below a comfortable reading size.
  ...
  return (
    <div className="min-w-[260px] w-[260px] flex flex-col bg-surface-container-lowest rounded-[18px] shadow-[0_4px_20px_rgba(0,0,0,0.05)] snap-start overflow-hidden cursor-pointer transition-transform active:scale-[0.98]">
      <div className="bg-cover bg-center w-full h-28 relative" style={{ backgroundImage: `url('${item.image}')` }}>
        <div className="absolute top-2 left-2 bg-surface/90 backdrop-blur-md px-2 py-1 rounded-full flex items-center gap-1 shadow-sm">
        ...
        {item.todayBadge && (
          <div className="absolute bottom-2 right-2 bg-primary text-on-primary px-2 py-1 rounded-lg shadow-sm">
...
export function KajianCardListItem({ item }) {
  ...
  return (
    <div className="flex items-center gap-3 p-2.5 bg-surface-container-lowest rounded-[18px] shadow-[0_4px_20px_rgba(0,0,0,0.05)] relative cursor-pointer transition-transform active:scale-[0.98]">
...
export function KajianCardSchedule({ item }) {
  ...
  return (
    <div className="bg-surface rounded-[20px] p-3 flex flex-col gap-2 shadow-[0_4px_20px_rgba(0,0,0,0.03)] relative overflow-hidden group">
      ...
      <div className="h-px w-full bg-outline-variant/30 my-0.5" />
